...
class CourseSearchSystem:
    ...
    def prepare_course_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        free_courses['search_text'] = free_courses.apply(
            lambda x: f"{x['Course Name']} {x['Key Takeaways']} {x['Difficulty']} {x['Ratings']} {x['Course Time']}", 
            axis=1
        )
    ...
    def search_courses(self, query: str, top_k: int = 5) -> List[Dict]:
        """Search for courses and return a list of results"""
        ...
        for idx in top_indices:
            ...
            results.append({
                ...
                'key_takeaways': course['Key Takeaways'],  # Correct field name
        ...
        return results  # Return list of results (instead of formatted response)
